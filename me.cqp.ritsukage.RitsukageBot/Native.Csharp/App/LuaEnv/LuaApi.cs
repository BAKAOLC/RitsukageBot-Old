...
namespace Native.Csharp.App.LuaEnv {
    class LuaApi {
        ...
        /// <returns>图片对象</returns>
        public static Bitmap PutText(Bitmap bmp,
            int x,
            int y,
            string text,
            string type = "宋体",
        ...
        /// <param name="b">b</param>
        /// <returns>图片对象</returns>
        public static Bitmap PutBlock(Bitmap bmp, int x, int y, int xx, int yy, int r = 0, int g = 0, int b = 0) {
            Color myColor = Color.FromArgb(r, g, b);
            //遍历矩形框内的各象素点
            for (int i = x; i <= xx; i++) {
                for (int j = y; j <= yy; j++) {
        ...
        public static Bitmap SetImage(Bitmap bmp, int x, int y, string path, int xx = 0, int yy = 0) {
            if (!File.Exists(path)) return bmp;
        ...
        public static string GetDir(Bitmap bmp) {
            string result = Tools.GetRandomString(
                32,
                true,
                false,
                false,
            ...
            bmp.Save(
                AppDomain.CurrentDomain.SetupInformation.ApplicationBase + "data/image/" + result + ".luatemp",
                ImageFormat.Jpeg
            );
        ...
        /// GET 请求与获取结果
        /// </summary>
        public static string HttpGet(string Url, string postDataStr = "", int timeout = 5000, string cookie = "") {
            try {
                //请求前设置一下使用的安全协议类型 System.Net
                if (Url.StartsWith("https", StringComparison.OrdinalIgnoreCase)) {
                    ServicePointManager.ServerCertificateValidationCallback = (sender, certificate, chain, errors) => {
        ...
        public static string GetVar(string n) {
            if (luaTemp.ContainsKey(n)) return luaTemp[n];
            return "";
        }
        ...
        //获取字符串的非转义形式
        public static string CqCode_ShareLink(string url, string title, string content, string imgUrl) =>
            Common.CqApi.CqCode_ShareLink(url, title, content, imgUrl);

        //获取酷Q "链接分享" 代码
        public static string CqCode_ShareCard(string cardType, long id) => Common.CqApi.CqCode_ShareCard(cardType, id);

        //获取酷Q "名片分享" 代码
        public static string CqCode_ShareGPS(string site, string detail, double lat, double lon, int zoom) =>
            Common.CqApi.CqCode_ShareGPS(site, detail, lat, lon, zoom);
        ...
        //获取酷Q "图片" 代码
        public static string CqCode_Music(long id, string type, bool newStyle) =>
            Common.CqApi.CqCode_Music(id, type, newStyle);

        //获取酷Q "音乐" 代码
        public static string
            CqCode_MusciDIY(string url, string musicUrl, string title, string content, string imgUrl) =>
            Common.CqApi.CqCode_MusciDIY(url, musicUrl, title, content, imgUrl);

        //获取酷Q "音乐自定义" 代码
        public static string CqCode_Record(string path) => Common.CqApi.CqCode_Record(path);

        //获取酷Q "语音" 代码
        public static int SendGroupMessage(long groupId, string message) =>
            Common.CqApi.SendGroupMessage(groupId, message);

        //发送群消息
        public static int SendPrivateMessage(long qqId, string message) =>
            Common.CqApi.SendPrivateMessage(qqId, message);

        //发送私聊消息
        public static int SendDiscussMessage(long discussId, string message) =>
            Common.CqApi.SendDiscussMessage(discussId, message);
        ...
        public static string CreateSocket(string name, string ip, int port) {
            return WebSocketManager.CreateSocket(name, ip, port);
        }

        public static void SendSocketMessage(string name, string msg) {
            WebSocketManager.SendSocketMessage(name, msg);
        }

        public static void CloseSocket(string name, string desc) {
            WebSocketManager.CloseSocket(name, desc);
        }

        //获取群成员信息
        public static int AddLoger(int level, string type, string content) =>
            Common.CqApi.AddLoger((LogerLevel) level, type, content);

        //添加日志
        public static int AddFatalError(string msg) => Common.CqApi.AddFatalError(msg);

        //添加致命错误提示
        public static int SetGroupWholeBanSpeak(long groupId, bool isOpen) =>
            Common.CqApi.SetGroupWholeBanSpeak(groupId, isOpen);

        //置全群禁言
        public static int SetFriendAddRequest(string tag, int respond, string msg) =>
            Common.CqApi.SetFriendAddRequest(tag, (ResponseType) respond, msg);

        //置好友添加请求
        public static int SetGroupAddRequest(string tag, int request, int respond, string msg) =>
            Common.CqApi.SetGroupAddRequest(tag, (RequestType) request, (ResponseType) respond, msg);

        //置群添加请求
        public static int SetGroupMemberNewCard(long groupId, long qqId, string newNick) =>
            Common.CqApi.SetGroupMemberNewCard(groupId, qqId, newNick);

        //置群成员名片
        public static int SetGroupManager(long groupId, long qqId, bool isCalcel) =>
            Common.CqApi.SetGroupManager(groupId, qqId, isCalcel);

        //置群管理员
        public static int SetAnonymousStatus(long groupId, bool isOpen) =>
            Common.CqApi.SetAnonymousStatus(groupId, isOpen);

        //置群匿名设置
        public static int SetGroupMemberRemove(long groupId, long qqId, bool notAccept) =>
            Common.CqApi.SetGroupMemberRemove(groupId, qqId, notAccept);
